...
class CalorieTrackerTableViewController: UITableViewController {
    ...
    private func locateFetchedObjectsAndPlaceInArray() {
        guard let fetchedCalorieObjects = fetchedResultController.fetchedObjects else { return }
        
        for fetchedObject in fetchedCalorieObjects {
            arrayOfCalories.append(fetchedObject.numOfCalories!)
            
        }
    }
    private func convertCalorieStringToDoubleAndCreateChart() {
        var numbersArray: [Double] = []
        
        for numOfCalories in arrayOfCalories {
            guard let convertedI = Double(numOfCalories) else { return }
        ...
        chartFor?.xLabelsFormatter = { String(Int($1)) + " hour"}
        chartFor?.yLabelsFormatter = { String(Int($1)) + " cals"}
